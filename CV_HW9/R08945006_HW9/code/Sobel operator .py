...
def Sobel(paddingimg, threshold):
    ...
    for row in range(0, shape[0]-2):
        for col in range(0, shape[1]-2):
            ...
            s1 = (paddingimg[row+2][col] + (2*paddingimg[row+2][col+1]) + paddingimg[row+2][col+2]) - (paddingimg[row][col] + (2*paddingimg[row][col+1]) + paddingimg[row][col+2])
            
            s2 = (paddingimg[row][col+2] + (2*paddingimg[row+1][col+2]) + paddingimg[row+2][col+2]) - (paddingimg[row][col] + (2*paddingimg[row+1][col]) + paddingimg[row+2][col])
...
cv2.imwrite('Sobel_t38.bmp',new_img)
